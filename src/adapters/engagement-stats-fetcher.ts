import { AppComponents, IEngagementStats, IEngagementStatsFetcher, ILimitsManager } from '../types'
import { ContractFactory, RequestManager } from 'eth-connect'
import { registrarContracts } from '@dcl/catalyst-contracts'
...
export async function createEngagementStatsFetcherComponent({
  config,
  ethereumProvider
}: Pick<AppComponents, 'config' | 'ethereumProvider'>): Promise<IEngagementStatsFetcher> {
  const networkId = await config.requireString('NETWORK_ID')
  const networkName = networkId === '1' ? 'mainnet' : 'goerli'
  const factory = new ContractFactory(new RequestManager(ethereumProvider), registrarAbi)
  const dclRegistrar = (await factory.at(registrarContracts[networkName])) as any
  ...
  return {
    async for(worldNames: string[]): Promise<IEngagementStats> {
      ...
      for (const worldName of worldNames) {
        try {
          const ownerOf = await dclRegistrar.getOwnerOf(worldName.replace('.dcl.eth', ''), 'latest')

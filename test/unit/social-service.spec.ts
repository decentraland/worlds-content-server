...
describe('SocialServiceAdapter', () => {
  ...
  describe('when isMemberFromCommunity is called', () => {
    describe('and the response is 204 (member)', () => {
      ...
      it('should return true', async () => {
        const result = await socialService.isMemberFromCommunity('0x1234', 'community-1')

        expect(result).toBe(true)
      })

      it('should call the correct endpoint', async () => {
        await socialService.isMemberFromCommunity('0x1234', 'community-1')

        expect(mockFetch.fetch).toHaveBeenCalledWith(`${socialServiceUrl}/v1/communities/community-1/members/0x1234`, {
          method: 'GET'
        })
      })

      it('should lowercase the address in the URL', async () => {
        await socialService.isMemberFromCommunity('0xABCD', 'community-1')
